...
初期実装では Raspberry Pi は観測基盤として先に立ち上げる。対象は Arduino USB 通信、`read` / `status` / `close` の Python クライアント、ローカル JSONL 保存、カメラ撮影・リサイズ、通常観測スケジューラ、GCP 送信失敗時のローカル退避までとする。
...
4. 送信前にローカル JSONL へ保存する
5. 画像を GCS に保存し、観測レコードを Firestore に保存する
6. この定期観測はデータ蓄積目的であり、水やり実行の直接トリガーにはしない
...
ただし初期実装段階では、`water` コマンド送信経路を設計に含めても本番では無効とする。流量測定、wet 閾値の実土壌校正、水道接続前安全試験が完了するまで、AI 判断から自動散水へは進めない。
